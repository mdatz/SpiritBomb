...
async function createNodeWorkerProcess(onMessage) {
  var _a2;
  (_a2 = parentPort) == null ? void 0 : _a2.on("message", async (msg) => {
    var _a3;
    (_a3 = parentPort) == null ? void 0 : _a3.postMessage(await onMessage(msg));
  });
}
...
import { routes as routes2, menus, cacheModules, trailingSlash as trailingSlash2, basePathname as basePathname2 } from "@qwik-city-plan";
...
async function requestHandler(requestCtx, render, platform, opts) {
  try {
    updateRequestCtx(requestCtx, trailingSlash2);
    const loadedRoute = await loadRoute(routes2, menus, cacheModules, requestCtx.url.pathname);
    if (loadedRoute) {
